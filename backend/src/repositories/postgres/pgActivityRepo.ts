...
/**
 * Internal helper: insert an activity log entry within an existing transaction.
 * Used by other postgres repos that need atomic activity logging.
 */
export async function pgLogActivity(
  tripId: string,
  ...
  await client.query(
    `INSERT INTO activity_log (trip_id, member_id, event_type, metadata)
     VALUES ($1, $2, $3, $4)`,
    [tripId, memberId ?? null, eventType, JSON.stringify(metadata)]
  );
}
